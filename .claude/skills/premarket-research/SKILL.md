...
description: Pre-market research (~13:30 CET / 7:30 ET, Mon-Fri). Screens the watchlist + market for day and swing candidates, computes indicators, reads overnight news, and produces a ranked candidate list with charts, proposed entry/stop/target, and risk-checked sizing. Then, for any strong-fit day-trade candidates (zero, one, or several that meet all criteria), prepares ready-to-send orders showing capital deployed vs available and projected profit-at-target / loss-at-stop, and can place them on explicit approval (human-in-the-loop — never auto-transmitted; swings stay research-only).
---
...
2. **Build the candidate universe — FULLY DYNAMIC, no hardcoded watchlist (2026-08-10).** The universe is searched live every run:
   ```python
   holdings = [p.contract.symbol for p in lib.ibkr.portfolio(ib)]
   universe = lib.screener.dynamic_universe(min_relvol=0, extra=holdings + lib.config.load_watchlist())
   ```
   `dynamic_universe()` unions, every call: (a) the **full-market relative-volume scan** (`market_scan`, TradingView-style: Price>$5, Mkt cap>$2B, Vol>1M, %change>3), (b) the **Yahoo Finance discovery screens** (gainers / most_active / growth_tech), and (c) the **near-real-time pre-market movers** (`premarket_movers`, Webull/yfinance). `extra=` folds in **current holdings** (always researched — tag held names so analysis covers managing the position, not just a fresh entry) plus any **optional user pins** in `config/watchlist.txt` (empty by default; there is NO hardcoded core list). **Pass `min_relvol=0` pre-market** — cumulative relvol under-reports before 9:30 ET; lean on %change/gap pre-open, re-apply relvol>2 at market-open. The IBKR scanner (`lib.data.scan`) tends to time out on this Gateway — skip gracefully.
   ...
   - **Watch for degraded discovery.** `dynamic_universe()`/`discover()` print `[screener] WARN: ...` to stderr if the scan/screens/movers returned 0 (API hiccup or feed down). If you see these, the universe has quietly shrunk to **holdings (+ any pins) only**. **State this explicitly at the top of the report** ("⚠ live market discovery unavailable this run — holdings only") so a thin candidate list isn't mistaken for a quiet market. Don't silently proceed as if you scanned the whole market.
...
4. **Apply the user's rule shortlist (hybrid).** These are the user's rules — apply them, don't substitute generic ones. Tag each candidate `day` or `swing`.
   ...
   **Day (5-min chart):**
   ...
   - **Scalp watch (added 2026-08-10).** The five scalps (`lib.scalps`) are **intraday** patterns that can't fully form pre-open, but flag each day candidate with the scalp profile it's set up for so `/market-open` knows what to watch: a name **extended 3+ ATR below the open on RVOL>5** → *rubber_band* watch; a name that **drove off a higher-TF level and is basing** → *hitchhiker*; a name **below VWAP that's carving higher-lows** → *back$ide*; a name **reclaiming a broken level** → *second_chance*. Note the level (`lib.indicators.level_ladder`) each would trigger against. The actual detect/entry happens at the open on 1-min bars via `lib.scalps.detect_all`.
...
8. **Report** to the user. The tables are research only; the **single exception** is the #1 day-trade pick in step 9, which is prepared as an order and placed **only on your explicit approval**. Lead with the scan board, then the two detail tables.
...
9. **Strong-fit day-trade orders — prepare ready order(s), show the money, place only on your OK (user rules, 2026-07-11).** Departure from the old "research-only" role: after the report, prepare ready-to-send orders for the day-trade candidates that are a **strong fit**, each with the capital + P&L math, and place them **only on explicit approval** (never auto-transmitted; CLAUDE.md's autonomy rule for opening trades is unchanged).
   - **Additive, never a filter:** the full step-8 report is unchanged — **every** day and swing candidate stays listed in the scan board and both research tables. This step only *adds* placeable orders on top; it never trims the research tables.
   - **Offer ONLY strong fits — quality over a count.** A name qualifies only if it clears **every** day gate with conviction: `day_trade_conditions()["score"] >= 5`, high **RVOL**, ATR% in the juice range, above VWAP (where formed) / clean pre-open level, a **legit driver (not a high-RVOL-no-news pump)**, and **R:R >= 2**. **It can be zero, one, or several** — never force a pick. If nothing qualifies, say "**no strong-fit day trade today — placing nothing**" and stop. **Swings stay research-only** here (act on them in `/market-open`).
   - **Levels (pre-open caveat):** VWAP/ORB aren't formed before 9:30 ET, so build the entry from **pre-market levels** — a limit at the premarket-high breakout / prior-day-high reclaim (buy the level, don't chase; entry ≤ lowest realistic fill). Protective stop = `lib.config.catastrophic_stop(entry)`, widened to clear `lib.indicators.noise_band(df, 1.5)`. Target from intraday levels (prior-day / premarket high or measured move); on a trend day use a 2R estimate labelled "trail — est."
   - **Show the money — pull available capital and offer two sizes.** `avail = lib.ibkr.account_value(ib, "AvailableFunds")`, `bp = lib.ibkr.account_value(ib, "BuyingPower")`. For each strong-fit pick present BOTH:
     - **Risk-capped (default):** `qty = lib.config.position_size(entry, stop, "day")` — honors `risk.yaml` (1.3% risk, 43%/name cap).
     - **Conviction / all-in:** `qty_max = floor(min(avail, bp) / entry)` — the most the account can fund (respect PDT + a single-name sanity check). The user has said they'll size up on a great setup, so **offer it**, but make the downside explicit.
   - **For each pick AND each size, show in the ask:** `shares | 💵 $ deployed (shares×entry) | % of available | entry | stop | target | R:R | 💰 profit if target hit ($ = shares×(target−entry) and %) | 🔻 loss if stopped ($ = shares×(entry−stop) and %)`. **Always show the loss figure — it is the real check on going all-in.** Verify `shares > 0` and R:R are real; enforce PDT + daily-loss limits.
   - **Build un-transmitted at the chosen size:** `contract, orders = lib.ibkr.prepare_entry(ib, sym, "BUY", qty, entry, stop)` (both `transmit=False`). Orders rest as pre-open limits that fill at/after the open; **`/market-open` step 1c re-validates them against live VWAP** and can cancel/re-adjust if the open invalidates the setup.
   - **Ask per pick:** "place <SYM>? which size — **risk-capped** (N sh / $X, risks $L) or **all-in** (M sh / $Y, risks $L2)? (yes+size / no)". **Transmit only on explicit approval:** `lib.ibkr.transmit(ib, contract, orders)`; log `status="approved"` via `lib.journal.log_trade(...)`. On "no"/silence, leave it un-placed. **Never transmit without the OK.**
