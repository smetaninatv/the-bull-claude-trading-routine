...
- **Autonomy:** Suggest + approve for **opening/adjusting risk**. Entry orders and target changes are built with `transmit=False` and sent only after the user OKs each one — never auto-transmit those. **Authorized exception (user, 2026-07-11): the unattended auto exit scan** (`scripts/auto_exit_scan.py`, scheduled every 15 min in RTH) may auto-transmit **stop raises** (always de-risking) **and breakeven-or-better closes** without approval — but it can **never realize a loss** (every close is gated on price ≥ avg cost; underwater names are left untouched) and never opens a position. All *manual* skills still ask before transmitting; this exception is scoped to that one headless script.
...
- **Runtime:** user's laptop, **not always on** → the *Claude skills* are **assisted/manual trigger** (user runs the skill when present), not unattended cron; don't assume a job fired on time. **Two local Windows Task Scheduler jobs run headless** (battery-tolerant, catch-up on): `Claude Trading Routine Reminders` (every 15 min → toast reminding which skill to run; DST/holiday-safe via `scripts/routine_reminder.py`) and `Claude Auto Exit Scan` (every 15 min in RTH → `scripts/auto_exit_scan.py --live`, the de-risk-only auto stop-raise + breakeven+ close on the live account). The order-placing/reporting skills can't be fully headless (approval / Claude reasoning needed); only the de-risk exit slice is.
